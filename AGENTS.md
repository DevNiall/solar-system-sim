...
- Moon data model: `src/app.js` builds `moonPivots`/`moonMeshes` per planet from `p.moons` (array; legacy singular `p.moon` still accepted) in `src/data.js`. Moon pivots are parented to the planet mesh, so a moon's on-screen orbit rate is planet `rotationSpeed` + moon `orbitSpeed`. Adding a moon is data-only; the sizing/spacing constraints that keep them legible (clear of the planet surface, outside Saturn's ring `outerRadius`, inside the gap to the next planet's orbit) are documented in the header comment of `src/data.js`. Moons are not clickable/selectable — per-moon facts live in the parent planet's `facts` array, which the info panel and quiz already surface.
